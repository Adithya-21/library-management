...
# --- AUTO-STOCK ENGINE: Updated Pricing & Resources ---
...
def payment_gateway(book_title, price, cat_key):
    ...
    st.markdown(f"<h3 style='color:#2563eb;'>Total Amount: ₹{price}</h3>", unsafe_allow_html=True)
    ...
    if st.button("Complete Transaction", key=f"pay_btn_{cat_key}", use_container_width=True):
        if len(card) >= 12:
            ...
            st.balloons(); st.success("Transaction Successful!"); return True
    ...
    with col:
        ...
        with t2:
            ...
            if st.button("Create Account", use_container_width=True):
                database.add_user(n_reg, e_reg, p_reg); st.success("Account created successfully!")
...
# ------------------ 5. MAIN APPLICATION ------------------
    ...
    if is_admin:
        ...
        st.sidebar.success("🛡️ Administrative Access")
    ...
    # A. DASHBOARD
    if choice == "📊 Dashboard":
        st.markdown("<div class='hero-bg'><h1>System Dashboard</h1></div>", unsafe_allow_html=True)
        books = database.get_all_books()
        c1, c2, c3 = st.columns(3)
        c1.metric("📚 Total Titles", len(books))
        c2.metric("📦 Inventory Stock", sum([b[4] for b in books]) if books else 0)
        c3.metric("⚡ Server Status", "🟢 Active")
            ...
            with tabs[i]:
                ...
                for idx, b in enumerate(data):
                    with cols[idx % 3]:
                        ...
                        if b[5] > 0: # Price based button logic
                        ...
                        if mode == "preview":
                            ...
                            if ok:
                                ...
                                if str(f_path).startswith("http"):
                                    st.link_button("🚀 Access Full Resource", f_path, use_container_width=True)
                                else:
    ...
    # C. LIBRARIAN DESK
        ...
        # System Audit Report
        if st.button("📥 Export System Audit (Excel)", use_container_width=True):
            ...
            with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
                df_books.to_excel(writer, sheet_name='Books_Inventory', index=False)
                df_users.to_excel(writer, sheet_name='User_Registry', index=False)
            st.download_button(label="📥 Download Excel File", data=buffer.getvalue(), 
                             file_name=f"Apex_Audit_{int(time.time())}.xlsx", 
        ...
        with st.form("add_book_form", clear_on_submit=True):
            ...
            if is_link: val = st.text_input("Resource URL")
            else: val = st.file_uploader("Full Resource (PDF)", type="pdf")
            ...
            if st.form_submit_button("🚀 Commit to Database", use_container_width=True):
                if t and a and pre_f and val:
                    ...
                    st.success(f"Resource '{t}' added successfully."); st.rerun()

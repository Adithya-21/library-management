...
st.markdown("""
    ...
    .book-card {
        ...
        margin-bottom: 15px; height: 170px;
            ...
            pdf_display = f'''
                <object data="data:application/pdf;base64,{b64}" width="100%" height="750px" type="application/pdf" style="border-radius:15px; border:2px solid #3b82f6;">
                    <div style="padding:40px; text-align:center; background:#f8f9fa; border-radius:15px;">
                        <h4>PDF Preview</h4>
                        <a href="data:application/pdf;base64,{b64}" download="preview.pdf" 
...
def payment_gateway(book_title, price, cat_key):
    ...
    if st.button("Unlock Full Book", key=f"final_pay_btn_{cat_key}"):
        if len(card) >= 12:
            ...
            st.balloons(); st.success("Access Granted!"); return True
        else: st.error("Invalid Payment Details")
...
if not st.session_state['auth']:
    st.markdown("<div class='hero-bg'><h1>🏛️ Apex Digital Library</h1><p>Modern Solutions for Modern Learning</p></div>", unsafe_allow_html=True)
        ...
        with t1:
            ...
            if st.button("Access Library", key="l_btn"):
                ...
                if user:
                    ...
                    st.rerun()
                else: st.error("Login Error. Please verify your credentials.")
        with t2:
            n_reg = st.text_input("Name", key="r_name")
            ...
            if st.button("Register Account", key="r_btn"):
                database.add_user(n_reg, e_reg, p_reg); st.success("Registration Successful!")
...
# ------------------ 5. MAIN APP ------------------
    ...
    # A. DASHBOARD
    if choice == "📊 Dashboard":
        st.markdown("<div class='hero-bg'><h1>Apex Management System</h1></div>", unsafe_allow_html=True)
    ...
    # B. EXPLORE CATALOG
        ...
        for i, cat in enumerate(genres):
            with tabs[i]:
                ...
                if not data: st.info("No resources found in this section.")
                
                cols = st.columns(3)
                for idx, b in enumerate(data):
                    with cols[idx % 3]:
                        st.markdown(f"<div class='book-card'><div class='book-title'>{b[1]}</div><div>{b[2]}</div><div style='font-size:0.8rem; color:#666;'>Available Copies: {b[4]}</div></div>", unsafe_allow_html=True)
                        c1, c2 = st.columns(2)
                ...
                if st.session_state['active_book']:
                    ...
                    if active_b:
                        ...
                        if mode == "preview":
                            ...
                            pay_ok = payment_gateway(active_b[1], active_b[5], cat) if mode == "pay" else True
                            if pay_ok:
                                f_path = active_b[7]
                                if str(f_path).startswith("http"):
                                    st.link_button("📥 Download (Cloud Storage)", f_path, use_container_width=True)
                                else:
    ...
    # C. LIBRARIAN DESK
        ...
        with st.form("add_book_form", clear_on_submit=True):
            ...
            cat = st.selectbox("Genre / Category", ["B.Tech", "Telugu", "Mythology"])
            
            col_q1, col_q2 = st.columns(2)
            stk = col_q1.number_input("Total Copies (Stock)", min_value=1, value=1)
            pr = col_q2.number_input("Setting Price (₹)", 0.0)
            
            use_link = st.checkbox("External Link Mode (For Drive files)")
            pre_f = st.file_uploader("Upload Preview (GitHub Folder)", type="pdf")
            if use_link: val = st.text_input("Direct Download Link")
            else: val = st.file_uploader("Upload Full PDF", type="pdf")
            ...
            if st.form_submit_button("🚀 Add Book to Database"):
                if t and a and pre_f and val:
                    ...
                    st.success(f"Success: '{t}' added with {stk} copies.")
                    st.rerun()
